...
1. Adicionar CDN no `<head>` se ainda não tiver:
   ...
   ```
2. **Bloco de defaults — OBRIGATÓRIO antes do primeiro `new Chart()`** (§5.0 de [../references/css-patterns.md](../references/css-patterns.md)). Sem ele o Chart.js renderiza na Helvetica default com formatação en-US ("1,234.5") — falha o validador (P5) e é o tell de IA mais barato de matar. Colar UMA vez, antes de qualquer gráfico:
   ```js
