...
1. **Esqueleto por script (não regurgitar):** `python scripts/scaffold.py deck <tema> outputs/<nome>.html` — injeta engine + layout + tema (~80% dos bytes) em disco, FORA do LLM. Depois preencha incremental (kit no slot `SLIDELESS:TYPE-KIT`, bloco parti, conteúdo seção-a-seção) com edits PEQUENOS — nunca o doc inteiro de uma vez. Ver [workflow.md §2–§4](../workflow.md).
2. Preencher o slot `SLIDELESS:TYPE-KIT` com o kit do parti ([../references/type-kits.md](../type-kits.md)) — **tema já injetado pelo scaffold**, não reaplicar. PROIBIDO Inter como display.
