...
2. Confirmar com o usuário:
   ...
   - **Tema** (7 em `assets/temas/temas.json`: itau-padrao/cream/navy/preto/grafite/areia + neutro; default `itau-padrao`).
...
1. **Esqueleto por script (não regurgitar):** `python scripts/scaffold.py handbook <tema> outputs/<nome>.html` — injeta engine + layout + tema (~80% dos bytes) em disco, FORA do LLM. Depois preencha incremental (kit no slot `SLIDELESS:TYPE-KIT`, bloco parti, conteúdo seção-a-seção) com edits PEQUENOS — nunca o doc inteiro de uma vez. Ver [workflow.md §2–§4](../workflow.md).
2. Preencher o slot `SLIDELESS:TYPE-KIT` com o `<link>` do kit ([../references/type-kits.md](../type-kits.md)) + bloco `:root` do kit antes do tema. **Tema já injetado pelo scaffold** (MARCA intacta) — só compor a camada DIREÇÃO se o parti pedir (edits pequenos).
